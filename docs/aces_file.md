...
> 生效范围：`pitch` 的 `user` / `delta` 层，以及
> `energy` / `air` / `falsetto` / `tension` 的 `user` 层。
> 这五个参数、这些层就是全部——其余层（如 `envelope`）传入会返回 `400`，见 3.2。
...
| 字段名  | 字段类型               | 是否必传 | 说明                                  |
|------|--------------------|------|-------------------------------------|
| user | Array(PIECE_VALUE) | 否    | 自定义参数线, 见 `piece_value` 对象，取值范围 0~1 |

**四个参数的 `user` 取值范围统一为 0~1**，数组中的负值表示"该处不指定，交给模型预测"。
超出 0~1 的值会被截断，不报错。

> **`envelope` 层已不再支持，传入会返回 `400`**（而不是静默忽略——静默会让您拿到
> `200` 却听不到自己画的起伏）。`envelope` 原本的语义是"对模型预测值做乘性修正"，
> 而调用方拿不到模型的预测值，服务端也无法把它等价折算成绝对曲线。
> 请改用 `user` 层直接给出 0~1 的绝对曲线。
