...
> 这 7 个维度分别设置不同的混合比例。当前引擎使用统一的音色表征，只有一路音色向量，
> 因此**只保留 `mel`**；传入其余 6 个维度中的任意一个都会返回 `400`。
>
> 之所以选择报错而不是忽略：静默接受会让您拿到 `200` 和一段音色不对的音频，
> 而无法从返回值察觉哪里出了问题。
>
> - 只需要单一歌手时，直接用 `speaker_id` 参数即可，无需构造 `mix_info`。
> - 想控制气息 / 假声 / 紧张度 / 力量的**强弱**，请用 ACES 文件里
>   `piece_params` 的 `air` / `falsetto` / `tension` / `energy` 曲线
>   （见[请求文件说明](/docs/aces_file.md) 第 3 节）。注意这与"借用另一位歌手的
>   该维度特征"不是一回事：前者是画一条 0~1 的强度曲线，后者当前引擎不支持。
...
1. **先把内容装进尽量少的文件**。单文件上限是 90 秒（见"4. 合成限制条件"），
   不要为了迁就早期的 18 秒限制而人为切碎——引擎按固定长度画布推理，合成 5 秒与
   合成 90 秒消耗的算力基本相同，切碎只会让同一段音频被反复推理。
