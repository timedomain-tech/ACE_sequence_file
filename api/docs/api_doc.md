...
| 限制piece合成的长度      | 20s       | 每个piece的长度不能超过限定时间                                    |
| socket接口限制整歌合成的长度 | 300s/600s | 最后一个note的结束时间减去第一个note的开始时间小于300s；最后一个note的结束时间小于600s |
| 引擎合成时间限制          | 30s       | 排队成功后，请求真正进入引擎合成的超时时间                                 |
| 请求队列长度            | 20        | 排队的请求超过限度后会直接返回503                                    |
| 每个token的qps限制     | 默认3，可联系调整 | 针对每个token可以设置并发度，最小 1req/s                            |
| 每个token的合成计费策略    | 用户选择      | 我们提供两种合成计费策略，按量和按时间，按量计费时合成成功计费一次                     |
